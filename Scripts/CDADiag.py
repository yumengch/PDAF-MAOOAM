...
def plotCorr():
    f = xr.open_dataset('../../netCDFFiles/maooamA0O0.nc', decode_times=False).isel(time=[360*200])
    coeffs = np.concatenate([f[varname+'_a'][:, 0].to_numpy() for varname in _varnames], axis=-1)
    f.close()
    print (coeffs.shape)
    fields = dict()
    for varname in _varnames:
        fields[varname] = np.zeros((_Ne, _nx, _ny), order='F')
    mod_model.mod_model.initialize_model()
    fields = toPhysical(coeffs, fields, '')
    mod_model.mod_model.finalize_model()
    i, j = _nx//2, _ny//2
    ...
    corr = dict()
    for varname in _varnames:
        A = fields[varname][:, i, j] - fields[varname][:, i, j].mean()
        AA = fields[varname] - fields[varname].mean(axis=0)
        AA = AA.reshape(_Ne, _nx*_ny, order='C')
        corr[varname] = (AA.T@A)/np.std(fields[varname][:, i, j],
                             ddof=1)/np.std(fields[varname],
                             ddof=1)/(_Ne - 1)
        print(corr[varname].shape)
        corr[varname] = corr[varname].reshape(_nx, _ny, order='C')
    np.savez('corr_weak_mid.npz', **corr)
    


if __name__ == '__main__':
    # gridpoints options: 'obs', 'unobs', 'all'
    saveError('obs', 'A0O0')
    saveError('unobs', 'A0O0')
    saveError('all', 'A1O1')
    saveError('all', 'A0O0')
    saveSpread('all', 'A0O0')
    saveSpread('all', 'A1O1')
