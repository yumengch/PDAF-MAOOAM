...
class FilterOptions:
    ...
    def __init__(self, config):
        ...
        self.filtertype = config.getint('filtertype', 6)
        # Subtype of filter algorithm
        self.subtype = config.getint('subtype', 0)
        # type of ensemble transformation
        self.type_trans = config.getint('type_trans', 0)
        # Type of transform matrix square-root
        # (0) symmetric square root, (1) Cholesky decomposition
        self.type_sqrt = config.getint('type_sqrt', 0)
        # (1) to perform incremental updating (only in SEIK/LSEIK!)
        self.incremental = config.getint('incremental', 0)
        ...
        self.covartype = config.getint('covartype', 1)
        # rank to be considered for inversion of HPH in analysis of EnKF
        # (0) for analysis w/o eigendecomposition
        # if set to >=ensemble size, it is reset to ensemble size - 1
        self.rank_analysis_enkf = config.getint('rank_analysis_enkf', 0)