...
class PDAFUserFuncs:
    ...
    def __init__(self, das):
        self.das = das
        self.state_p_background = np.zeros(self.das.sv.dim_state_p)
        self.firsttime = True
    ...
    def prepoststep_ens_pdaf(self, step, dim_p, dim_ens, dim_ens_p,
        ...
        if step < 0: self.state_p_background = state_p.copy()

        if step > 0:
            nx, ny = self.das.model.nx, self.das.model.ny
            offset = 0 if 'obso' in self.das.obs.obs else 2*nx*ny
            anomaly = ens_p[offset:offset+2*nx*ny] - state_p[offset:offset+2*nx*ny, None]
            state_p[offset:offset+2*nx*ny] = \
                (1 - np.sqrt(self.das.infl.forget))*self.state_p_background[offset:offset+2*nx*ny] + \
                np.sqrt(self.das.infl.forget)*state_p[offset:offset+2*nx*ny]

            ens_p[offset:offset+2*nx*ny] = \
                   state_p[offset:offset+2*nx*ny, None] + \
                   np.sqrt(self.das.infl.forget)*anomaly
