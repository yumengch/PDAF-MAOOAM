...
class filter_options:
    ...
    def __init__(self, config: configparser.SectionProxy) -> None:
        ...
        self.filtertype:int = config.getint('filtertype', 6)
        # Subtype of filter algorithm
        self.subtype:int = config.getint('subtype', 0)

        # type of forgetting factor
        # - (0) fixed
        # - (1) global adaptive
        # - (2) local adaptive for LSEIK/LETKF/LESTKF
        self.type_forget:int = config.getint('type_forget', 0)
        # forgeting factor
        self.forget:float = config.getfloat('forget', 1)

        # Set other parameters to default values
        self.type_trans:int = config.getint('type_trans', 0)
        self.type_sqrt:int = config.getint('type_sqrt', 0)
        self.incremental:int = config.getint('incremental', 0)
        self.covartype:int = config.getint('covartype', 1)

        self.rank_analysis_enkf:int = config.getint('rank_analysis_enkf', 0)
